...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se bi gioi han 2 gio choi dau cho nguoi moi tham gia cong dong TSP-CITY.");
		}
		/*if(PlayerInfo[i][pLevel] >= 1) {
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Thong Bao: Ban Chua Co Giay Phep Su Dung xe , thuyen , may bay , Hay Nhap /MAP => Tim DUong => Thi Bang Lai De Co Bang Lai nhe !");
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Neu Ban Da Co Bang Lai Hay Bo Qua Tin Nhan Nay Nhe !");
		}*/
		if(PlayerInfo[i][pLamNhiemVu][0] == 0 || PlayerInfo[i][pLamNhiemVu][1] == 0 || PlayerInfo[i][pLamNhiemVu][2] == 0 || PlayerInfo[i][pLamNhiemVu][3] == 0 || PlayerInfo[i][pLamNhiemVu][4] == 0) {
			SendClientMessageEx(i, COLOR_YELLOW, "Ban chua lam xong nhiem vu tan thu [/nvtanthu]");
		}
	...
	foreach(new i: Player)
	{
	    ...
	    if(GetPVarType(i, "Injured"))
	    {
	        ...
	        if(GetPVarInt(i, "EMSAttempt") != 0)
			{
				...
				new Float:health;
				new string[128];
				format(string, sizeof(string), "(( %s (%d) dang bi thuong , hay /call 911 de goi bac si cuu anh ta ))", GetPlayerNameEx(i), i);
				TogglePlayerControllable(i, 0);
				GameTextForPlayer(i, "~r~Bi Thuong~n~~w~Dang cho Cap Cuu...", 5000, 3);
 				GetPlayerHealth(i,health);
				SetPlayerHealth(i, health-1);
				if(GetPVarInt(i, "EMSAttempt") == -1)
				{
					if(GetPlayerAnimationIndex(i) != 1701) ClearAnimations(i), ApplyAnimation(i, "SWEET", "Sweet_injuredloop", 4.1, 1, 1, 1, 1, 1, 1);
					...
					GameTextForPlayer(i, "~r~Ban Da Bi Thuong ~n~~w~/dichvu capcuu", 5000, 3);
				}
				if(GetPVarInt(i, "EMSAttempt") == 1)
				{
					if(GetPlayerAnimationIndex(i) != 1701) ClearAnimations(i), ApplyAnimation(i, "SWEET", "Sweet_injuredloop", 4.1, 1, 1, 1, 1, 1, 1);
...
task ServerHeartbeat[1000]() {
	...
	foreach(new i: Player)
	{
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0) {
				if(strfind(PlayerInfo[i][pPrisonReason], "[IC]", true) != -1 || strfind(PlayerInfo[i][pPrisonReason], "[ISOLATE]", true) != -1) {
					...
					PlayerInfo[i][pVW] = 0;
					SetPlayerPos(i, -33.8530,1507.7990,12.7560);
				}
				else {
				 	SetPlayerInterior(i, 0);
					PlayerInfo[i][pInt] = 0;
				...
				PhoneOnline[i] = 0;
				SendClientMessageEx(i, COLOR_GRAD1,"[TSP CITY] Ban da tra no cho xa hoi.");
		...
		if (PlayerInfo[i][pAdmin] < 2 || HelpingNewbie[i] != INVALID_PLAYER_ID)
		{
			if (PlayerInfo[i][pHospital] == 0 && GetPVarInt(i, "Injured") != 1 && GetPVarInt(i, "IsFrozen") == 0 && GetPVarInt(i, "PlayerCuffed") == 0)
			{
				if (++PlayerInfo[i][pHungerTimer] >= 1000 && PlayerInfo[i][pHunger] > 0) // 30 minutes
				{
					PlayerInfo[i][pHungerTimer] = 0;
					PlayerInfo[i][pHunger] -= 5;
					SendClientMessageEx(i, COLOR_LIGHTRED, "Ban bi tru 5 diem doi bung , /thongtin");
					...
					if (PlayerInfo[i][pHunger] == 0)
					{
						...
						RemovePlayerFromVehicle(i);
		 				...
		 				SetTimerEx("thoigiandoibung", 30000, false, "i", i);
		 				TogglePlayerControllable(i, 0);
						SendClientMessageEx(i, COLOR_LIGHTGREEN, "Ban dang rat doi bung -  ban se tu vong trong vong 5 phut nua, ban can phai an!");
						//SendClientMessageEx(i, COLOR_LIGHTGREEN, "Neu chua co do an hay mau di che bien hoac mua tu nguoi khac");
					}
				}
				if(PlayerCuffed[i] == 0)
				{
					if (PlayerInfo[i][pHunger] == 0 && ++PlayerInfo[i][pHungerDeathTimer] >= 300) // 10 minutes
					{
...
task ServerMicrobeat[500]() {
	...
	//format(str_time, sizeof(str_time), "%d/%s/%d - %d:%02d~n~|TSP-CITY.NET|", day, mtext, year, thour, minuite);
