...
#define TIMER_ITEM_WORLD        1000*1000
...
new Itens[][enum_Itens] =
{
    ...
    {1,     ITEM_TYPE_HELMET,       18645,      "Non Bao Hiem",             1,          true,       {0.000000, 0.000000, 0.000000, 1.000000},       "Protege contra headshots."},
    ...
    {3,     ITEM_TYPE_WEAPON,       356,        "M4",                   1,          true,       {0.000000, -30.00000, 0.000000, 2.200000},      "Fuzil de longo alcance~n~com m�dia precis�o."},
    {4,     ITEM_TYPE_AMMO,         2061,       "Ammo",                200,        true,       {0.000000, 0.000000, 0.000000, 2.000000},       "Muni��o para armas de fogo."},
    ...
    {7,     ITEM_TYPE_BACKPACK,     3026,       "Balo Cap 2",       1,          true,       {0.000000, 0.000000, 0.000000, 1.000000},       "Mochila que aumenta seu~n~invent�rio."},
    {8,     ITEM_TYPE_BACKPACK,     3026,       "Balo Cap 3",       1,          true,       {0.000000, 0.000000, 0.000000, 1.000000},       "Mochila que aumenta seu~n~invent�rio."},
    {9,     ITEM_TYPE_WEAPON,       355,        "AK-47",                1,          true,       {0.000000, -30.00000, 0.000000, 2.200000},      "Fuzil de longo alcance~n~com m�dia precis�o."},
    ...
    {11,    ITEM_TYPE_MELEEWEAPON,  335,        "Knife",                 1,          true,       {0.000000, -30.00000, 0.000000, 2.200000},      "Arma de corpo-a-corpo."},
    {12,    ITEM_TYPE_MELEEWEAPON,  334,        "Baton",             1,          true,       {0.000000, -30.00000, 0.000000, 1.500000},      "Arma de corpo-a-corpo."},
    {13,    ITEM_TYPE_WEAPON,       352,        "Uzi",                  1,          true,       {0.000000, -30.00000, 0.000000, 1.200000},      "Micro metralhadora~n~de duas m�os.."},
    ...
    {18,    ITEM_TYPE_NORMAL,       11738,      "Binh Cuu Thuong",          5,          true,       {0.000000, 0.000000, 0.000000, 1.000000},       "Kit de primeiro socorros~n~que recupera sua vida."}
};
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    else if(clickedid == inventario_split[0])
    {
        ...
        if(IsInventoryFull(playerid))
            ...
            "~r~ERRO: ~w~Seu invent�rio est� cheio.",
        ...
        if(pInventory[playerid][invSlotAmount][slot] == 1)
            return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
            "~r~ERRO: ~w~Voc� n�o pode dividir esse item.",
    ...
    else if(clickedid == inventario_drop[0])
    {
        ...
        if(!Itens[itemid][item_canbedropped])
            return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
            "~r~ERRO: ~w~Voc� n�o pode derrubar esse item.",
    ...
    else if(clickedid == inventario_remover)
    {
        ...
        if(selected == 2)
            if(GetSlotsInUse(playerid) > 5)
                return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
                "~r~ERRO: ~w~Esvazie seu invent�rio.",
                "~r~ERROR: ~w~Clean your inventory."));

        if(selected == 2)
            if(GetSlotsInUse(playerid) >= 5)
                return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
                "~r~ERRO: ~w~N�o tem espa�o no seu invent�rio.",
    ...
    else if(Itens[item][item_tipo] == ITEM_TYPE_WEAPON || Itens[item][item_tipo] == ITEM_TYPE_MELEEWEAPON)
    {
        if(GetAmmunation(playerid) <= 0 && Itens[item][item_tipo] == ITEM_TYPE_WEAPON)
        {
            if(item != 4 && item != 17)
            {
                ...
                return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
                "~r~ERRO: ~w~Voc� n�o tem muni��o.", "~r~ERROR: ~w~You don't have ammunation."));
            }
        }
        ...
        if(weapons[weaponslot][0] != 0 && weapons[weaponslot][1] > 0)
        {
            pInventory[playerid][invSelectedSlot] = slot;
            return ShowMessageInventory(playerid, Translate(Player[playerid][Language],
            "~r~ERRO: ~w~N�o � possivel equipar duas armas do mesmo tipo.",
        ...
        for(new i = 3; i < 7; i ++)
        {
            if(pCharacter[playerid][charSlot][i] == item)
            {
                pInventory[playerid][invSelectedSlot] = slot;
                ShowMessageInventory(playerid, Translate(Player[playerid][Language],
                "~r~ERRO: ~w~N�o � possivel equipar duas armas iguais.",
        ...
        if(!have_slot)
        {
            pInventory[playerid][invSelectedSlot] = slot;
            ShowMessageInventory(playerid, Translate(Player[playerid][Language],
            "~r~ERRO: ~w~N�o � possivel equipar mais armas.",
...
stock ShowInventory(playerid)
{
    ...
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][0], Translate(Player[playerid][Language], "Personagem", "Nhan Vat"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][1], Translate(Player[playerid][Language], ConvertToGameText("Seu Invent�rio"), "Tui Do"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][2], Translate(Player[playerid][Language], ConvertToGameText("Cabe�a"), "Dau"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][3], Translate(Player[playerid][Language], "Mochila", "Balo"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][4], Translate(Player[playerid][Language], "Corpo", "Than Hinh"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][5], Translate(Player[playerid][Language], "Arma", "Vu Khi"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][6], Translate(Player[playerid][Language], "Arma", "Vu Khi"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][7], Translate(Player[playerid][Language], "Arma", "Vu Khi"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][8], Translate(Player[playerid][Language], "Arma", "Vu Khi"));
    PlayerTextDrawSetString(playerid, inventario_textos[playerid][9], Translate(Player[playerid][Language], "Usar", "Su Dung"));
...
CMD:themdo(playerid, params[])
{
    new
        id, item, amount;

    if(sscanf(params, "uii", id, item, amount))
        return SendClientMessage(playerid, -1, "/themdo <id/nick> <id do item> <quantidade>");
...
stock LoadTextDraws()
{
    ...
    inventario_split[1] = TextDrawCreate(540.294372, 334.449981, "Tach");
    ...
    inventario_drop[1] = TextDrawCreate(540.762878, 355.451263, "Vut Bo");
...
LoadPlayerTextDraws(playerid)
{
    ...
    inventario_textos[playerid][0] = CreatePlayerTextDraw(playerid, 68.199996, 120.716636, "Nhan Vat");
    ...
    inventario_textos[playerid][1] = CreatePlayerTextDraw(playerid, 315.710540, 120.716636, ConvertToGameText("Tui Do"));
    ...
    inventario_textos[playerid][2] = CreatePlayerTextDraw(playerid, 248.200164, 144.800033, ConvertToGameText("Dau"));
